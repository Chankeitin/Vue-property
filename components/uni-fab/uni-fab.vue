...
<script>
	...
	export default {
		...
		computed: {
			contentWidth(e) {
				return (this.content.length + 1) * 75 + 10 + 'px'
			...
			// 动态计算宽度
			boxWidth() {
				return this.getPosition(3, 'vertical','width')
			},
			// 动态计算高度
			boxHeight() {
				return this.getPosition(3, 'vertical','height')
			},
			// 计算左下位置
			leftBottom() {
		...
		methods: {
			...
			getPosition(types, paramA, paramB) {
				if (types === 0) {
					...
					if (paramB === 'width')
						return this.isShow && this.direction === paramA ? '100px' : this.contentWidthMin
				
					if (paramB === 'height') 
						return this.isShow && this.direction === paramA ? '180px' : this.contentWidthMin
				}		
			}
		}
	}
